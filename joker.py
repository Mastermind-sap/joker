...
async def gifs(ctx, subreddit):  # pylint: disable=missing-function-docstring
    if subreddit is not None:
        title, text, url = redditapi.gifs(val=subreddit)
    else:
        title, text, url = redditapi.gifs()
    await ctx.send(title)
    await ctx.send(url)
    await ctx.send(text)

@bot.commands()
async def memes(ctx, subreddit):  # pylint: disable=missing-function-docstring
    if subreddit is not None:  # Put an optional subreddit for fetching memes
        title, text, url = redditapi.memes(val=subreddit)
    else:   # default subreddit = linuxmemes
